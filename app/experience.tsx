...
export default function Experience() {
  ...
  const showSample=()=>{const a=sampleAnswers();setAnswers(a);setReport(scoreAssessment(a));setIsSample(true);setView('report');window.scrollTo(0,0)};
  ...
  const importAnswers=async(file?:File)=>{if(!file)return;try{const parsed=JSON.parse(await file.text());if(parsed.format!=='starseed-assessment-answers'||parsed.version!==1||typeof parsed.answers!=='object')throw new Error('檔案格式或版本不正確');const missing=questions.filter((q:any)=>parsed.answers[q.id]===undefined).map((q:any)=>q.id);if(missing.length)throw new Error(`缺少 ${missing.length} 題：${missing.slice(0,6).join('、')}${missing.length>6?'…':''}`);setAnswers(parsed.answers);setReport(scoreAssessment(parsed.answers));setIsSample(false);setView('report');setNotice('答案已匯入並完成判讀');window.scrollTo(0,0);}catch(e:any){setNotice(`無法匯入：${e.message}`)}};
...
function Quiz({questions,index,answers,seed,ranked,setRanked,onAnswer,onBack}:any){
  ...
  return <main className="quiz-shell min-h-screen text-white"><header className="quiz-header"><button onClick={onBack} aria-label="上一題"><ArrowLeft/></button><div><Progress value={progress} className="quiz-progress"/><span>{index+1} / {questions.length}</span></div><span className="size-10"/></header><section className="question-wrap"><div className="question-number">QUESTION {String(index+1).padStart(2,'0')}</div><h1>{q.question}</h1>{q.intro&&<p className="question-intro">{q.intro}</p>}
...
function Report({report,sample,onHome,onAtlas,onDetail,onExport,onRestart,onShare}:any){const primary=report.ranking[0];const story=insights[primary.code];const radar=data.dimensions.map((name:string,i:number)=>({name:name.slice(0,4),value:Math.round(((report.dimensions[i]??0)+1)*50)}));return <main className="report-shell min-h-screen text-white"><div className="px-5 pt-5 sm:px-8"><Header onHome={onHome} onAtlas={onAtlas}/></div><section className="report-hero"><div className="report-icon"><img src={primary.icon} alt={`${primary.name}象徵`}/></div><div className="report-hero-copy">{sample&&<span className="sample-chip">報告範例</span>}<p className="section-kicker">你的星際血脈</p><h1>{report.declaration.title}</h1><h2>{report.declaration.subtitle}</h2><p>{declarations[primary.code]}</p><div className="hero-stats"><Metric label="文明契合" value={primary.final} suffix=""/><Metric label="身份共振" value={report.gate}/><Metric label="分類信心" value={report.confidence}/></div></div></section>
  ...
  <section className="report-section"><p className="section-kicker">你的文明核心</p><h2>天賦會照亮道路，陰影會鍛造力量</h2><div className="gift-grid"><article><span>LIGHT</span><h3>你攜帶的天賦</h3>{story.gifts.map((x:string)=><p key={x}><Sparkles/>{x}</p>)}</article><article><span>SHADOW</span><h3>正在進化的陰影</h3>{story.shadows.map((x:string)=><p key={x}><Orbit/>{x}</p>)}</article></div><blockquote>{story.key}</blockquote></section>
